...
- train.csv = le2i_train + ofsyn(sample by video, default 80%)
- test.csv  = le2i_test  + ofsyn(sample by video, default 20%)

By default, class `Other` keeps only 50% of sampled OF-SYN videos
(so with 80/20 global ratios it becomes 40/10 for `Other`).
...
MAX_VALID_LABEL_ID_8CLS = len(CLASS_NAMES) - 1

# OF-SYN uses "walking" in video prefixes while some mappings use "walk".
SOURCE_TO_TARGET_NAME_10_TO_8_NORMALIZED = {
	key.strip().lower(): value for key, value in SOURCE_TO_TARGET_NAME_10_TO_8.items()
}
SOURCE_TO_TARGET_NAME_10_TO_8_NORMALIZED["walking"] = "Walking"
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		"--le2i-train-csv",
		default="/home/cxviewlab2/data/khoa.do/CTR-GCN/data/le2i_ofsyn/combine/csv/10_to_8_100_v2/csv_v2/le2i_train_yolo11m.csv",
		help="Remapped le2i train CSV",
	)
	parser.add_argument(
		"--le2i-test-csv",
		default="/home/cxviewlab2/data/khoa.do/CTR-GCN/data/le2i_ofsyn/combine/csv/10_to_8_100_v2/csv_v2/le2i_test_yolo11m.csv",
		help="Remapped le2i test CSV",
	)
	parser.add_argument(
		"--ofsyn-csv",
		default="/home/cxviewlab2/data/khoa.do/CTR-GCN/data/le2i_ofsyn/combine/csv/10_to_8_100_v2/csv_v2/ofsyn_yolo11m_with_flip.csv",
		help="OF-SYN CSV",
	)
	parser.add_argument(
		"--output-dir",
		default="./csv/10_to_8_100_v2_flip",
	...
	parser.add_argument(
		...
		type=float,
		default=1.0,
		help="Scale only class `Other` sampling ratios (0.5 means keep 50% of Other videos)",
	)
	parser.add_argument(
		"--max-label-id",
		type=int,
		default=MAX_VALID_LABEL_ID_8CLS,
...
def parse_label_id(raw: str) -> Optional[int]:
	try:
...
def is_label_kept(label_id: Optional[int], max_label_id: int) -> bool:
...
OFSYN_CLASS_PREFIXES = sorted([
    "sit_down", "lie_down", "stand_up",
    "walking", "walk",          # thêm "walking" và để trước "walk" (longest first)
    "fall", "fallen", "sitting",
    "lying", "standing", "other",
], key=len, reverse=True)

def get_ofsyn_source_class(row: dict[str, str]) -> Optional[str]:
    video = str(row.get("video", "")).strip()
    
    if video.startswith("ofsyn_"):
        suffix = video[len("ofsyn_"):]  # "sit_down_video001"
        for cls in OFSYN_CLASS_PREFIXES:
            if suffix.startswith(cls + "_") or suffix == cls:
                return cls
    
    if "/" in video:
        return video.split("/", 1)[0].strip().lower()
...
def map_ofsyn_to_le2i(row: dict[str, str]) -> Optional[Tuple[str, int]]:
	source_class = get_ofsyn_source_class(row)
	if source_class is None:
		return None

	target_class = SOURCE_TO_TARGET_NAME_10_TO_8_NORMALIZED.get(source_class)
...
def sample_videos_by_class(
	...
	for class_name in sorted(class_to_videos):
		...
		total = len(videos)
		class_train_ratio = train_ratio
		class_test_ratio = test_ratio
		if class_name == "Other":
			# Downsample the Other class only (e.g. 80/20 -> 40/10 when keep=0.5).
			class_train_ratio *= other_keep_ratio
			class_test_ratio *= other_keep_ratio

		n_train = int(round(total * class_train_ratio))
		n_train = max(0, min(n_train, total))

		remain = total - n_train
		n_test = int(round(total * class_test_ratio))
...
def main() -> None:
	...
	ofsyn_train_videos, ofsyn_test_videos, stats = sample_videos_by_class(
		...
		other_keep_ratio=float(args.other_keep_ratio),
		seed=int(args.seed),
	)
	...
	print(f"other keep ratio      : {args.other_keep_ratio:.2f} (class Other only)")
	print(f"ofsyn sampled videos  : train={len(ofsyn_train_videos)}, test={len(ofsyn_test_videos)}")
	print(f"ofsyn dropped rows (class not in LE2I-8 before sampling): {ofsyn_dropped_before_sampling}")
	...
	print(f"  dropped by filter: le2i_train={le2i_train_dropped}, le2i_test={le2i_test_dropped}, ofsyn_not_in_le2i8={ofsyn_dropped_rows}")
